...
export default function MenuItem({ name, description, price, category, isSpecial = false, isPopular = false, image }: MenuItemProps) {
  return (
    <div className={`group relative overflow-hidden rounded-lg sm:rounded-xl transition-all duration-500 hover:scale-105 ${isSpecial
      ...
      <div className="relative p-4 sm:p-6">
        ...
        {image && (
          <div className="mb-4 relative">
            <div className="w-full h-32 sm:h-40 md:h-48 bg-gradient-to-br from-green-800 to-green-900 rounded-lg overflow-hidden">
              <Image
                src={image}
                alt={name}
                width={400}
            ...
            {isSpecial && (
              <div className="absolute top-2 left-2">
                <span className="bg-amber-500 text-green-900 px-2 py-1 rounded-full text-xs font-bold shadow-lg">
                  SPECIAL
                </span>
              </div>
            )}
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-1">
              <h4 className={`text-lg sm:text-xl font-bold ${isSpecial ? 'text-amber-100' : 'text-amber-100'
                }`}>
                {name}
              </h4>
              {isPopular && !image && (
          ...
          <div className="flex flex-col items-end">
            <span className={`text-xl sm:text-2xl font-bold ${isSpecial ? 'text-amber-400' : 'text-amber-400'
              }`}>
              {price}
            </span>
            {isSpecial && !image && (
        ...
        <p className="text-amber-200/80 text-sm sm:text-base leading-relaxed font-light">
          {description}
        </p>
